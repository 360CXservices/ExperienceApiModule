...
namespace VirtoCommerce.XDigitalCatalog.Schemas
{
    ...
    {
         Task<CatalogProduct> GetAllAsync(string Id);
    }
    public class GetProperties : IGetProperties
    {
        private readonly IItemService _itemsService;
        public GetProperties(IItemService itemsService)
        {
            _itemsService = itemsService;
        }

        public  async Task<CatalogProduct> GetAllAsync(string Id)
        {
           return await _itemsService.GetNoCloneAsync(Id, null);
        }
    }
        ...
        public ProductType(IMediator mediator, IDataLoaderContextAccessor dataLoader)
        {
            ...
            ExtendableField<ListGraphType<PropertyType>>("keyProperties",
                ...
                resolve: async context =>
                {
                    ...
                    var variations = response.Products.Select(expProduct => new ExpVariation(expProduct)).ToList();

                    var response1 = new List<Property>();
                    foreach (var item in variations)
                    {
                        ...
                        var res = item.IndexedProduct.Properties;
                        foreach (var property in res)
                        {
                            
                            response1.Add(property);
                        }
                    ...
                    if (!names.IsNullOrEmpty())
                    {
                        response1 = response1.Where(x => names.Contains(x.Name, StringComparer.InvariantCultureIgnoreCase)).ToList();
                    }
